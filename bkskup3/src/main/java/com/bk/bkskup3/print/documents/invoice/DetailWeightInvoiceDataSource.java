 ...
 * Time: 9:20 PM
 */
public class DetailWeightInvoiceDataSource extends AbstractInvoiceDataSource {
    public DetailWeightInvoiceDataSource(DocumentContext context, DocumentProfile profile) {
    ...
    protected Collection<InvoiceItem> createItems() {
        ...
        for (CowCalculator cowCalc : invoiceCalc.getCowCalculators()) {
            ...
            item.setGoodsQty(Numbers.formatWeight(cow.getWeight()));
            item.setGoodsQtyUnit(units.getUnit(Unit.KG));
            ...
            item.setUnitPriceNet(pricePegKgFormat.format(cowCalc.getNetPricePerKg()));
